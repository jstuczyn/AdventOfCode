...
impl FromStr for VentLine {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let start = coords.next().context("malformed vent line")?;
        let mut x_y1 = start.split(',');
        let x1 = x_y1.next().context("malformed vent line")?.parse()?;
        let y1 = x_y1.next().context("malformed vent line")?.parse()?;

        let end = coords.next().context("malformed vent line")?;
        let mut x_y2 = end.split(',');
        let x2 = x_y2.next().context("malformed vent line")?.parse()?;
        let y2 = x_y2.next().context("malformed vent line")?.parse()?;
...
pub fn part1(input: Vec<VentLine>) -> usize {
...
pub fn part2(input: Vec<VentLine>) -> usize {
    let mut coverage: HashMap<_, i32> = HashMap::new();
