...
impl FromStr for Edge {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let from = Node::new(nodes.next().context("malformed edge")?);
        let to = Node::new(nodes.next().context("malformed edge")?);
        Ok(Edge { from, to })
    }
}

pub fn part1(input: Vec<Edge>) -> usize {
...
pub fn part2(input: Vec<Edge>) -> usize {
    let graph = Graph::construct(&input);
    let start = Node {
        name: "start".to_owned(),
        is_big: false,
