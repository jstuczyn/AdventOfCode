...
fn p2_visit_entry(entry: &Rc<RefCell<Bag>>) -> usize {
    let mut branch_bags = 1;

    for child in &entry.borrow().inner {
        branch_bags += child.count * p2_visit_entry(&child.bag);
    }
...
fn parse_rule(rule: &str) -> (String, Vec<BagInnerRaw>) {
    ...
    let inner_bags = split[1].split([',', '.']);
    let raw_inner = inner_bags
        .map(str::trim)
        .filter(|bag| !bag.is_empty())
        .filter_map(into_count_and_name)
        .collect();
