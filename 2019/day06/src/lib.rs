...
impl OrbitalMap {
    pub fn new(raw_orbits: Vec<Orbit>) -> Self {
...
impl Orbit {
    ...
    fn extract_orbiting_object_details(&mut self, orbit_directory: &mut HashMap<String, Orbit>) {
        for orbiting_object in &mut self.orbiting_objects {
    ...
    fn combine(&mut self, other: Orbit) {
        ...
        self.orbiting_objects.extend(other.orbiting_objects);
    }
...
pub fn parse_orbits(raw_orbits: Vec<String>) -> Vec<Orbit> {
    raw_orbits
        .into_iter()
        .map(|raw_orbit| {
...
pub fn do_part1(orbital_map: &OrbitalMap) {
    println!("Part 1 answer: {}", orbital_map.total_orbit_count());
}

pub fn do_part2(orbital_map: &OrbitalMap) {
