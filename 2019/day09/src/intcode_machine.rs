...
impl Tape {
    ...
    fn write(&mut self, position: usize, value: isize) {
        if position >= self.0.len() {
            // according to day09 specs, write should always succeed (unless to negative index)
    ...
    fn read(&mut self, position: usize) -> isize {
        if position >= self.len() {
            // according to day09 specs, read should always succeed (unless on negative index)
            self.resize(position + 1);
        }
        ...
        match param_mode {
            ...
            ParamMode::Relative => {
                if position as isize + relative_base < 0 {
                    return Err(TapeError::WriteOutOfRange);
                }
                self.write((position as isize + relative_base) as usize, value);
                Ok(())
            }

            ParamMode::Immediate => Err(TapeError::WriteInImmediateMode),
        }
    }
}
...
mod tests {
    ...
    //     let mut day2_tape = Tape::new(utils::read_input_file("inputs/2019/day02"));
    ...
    //     let tape = Tape::new(utils::read_input_file("inputs/2019/day05"));
    //
    //     let input = b"1";
